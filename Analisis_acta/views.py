...
def calculo_novedades_acta(request):
    ...
    for pedido in pedidos:
        ...
        if pedido.pedido not in doblealinea:
            pedidos_doble_linea(pedido)
            doblealinea.append(pedido.pedido)
        ...
        if primera_letra == 'B':
            if pedido.item_cont == 'B 03':
                ...
                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 04')).count() == 0) and (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 06')).count() == 0) and (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 07')).count() == 0):
                    crear_novedad(pedido, 'B03 sin B 04, B 06 y B 07')
            ...
            if pedido.item_cont == 'B 04':
                ...
                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 03')).count() == 0):
                    crear_novedad(pedido, 'B 04 sin B 03')

            if pedido.item_cont == 'B 05':
                if int(pedido.cantidad) > 0:
                    crear_novedad(pedido, 'B 05 > 0')

            if pedido.item_cont == 'B 06':
                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 03')).count() == 0):
                    crear_novedad(pedido, 'B 06 sin B 03')
                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 04')).count() == 0):
                    crear_novedad(pedido, 'B 06 sin B 04')
            ...
            if pedido.item_cont == 'B 07':
                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 03')).count() == 0):
                    crear_novedad(pedido, 'B 07 sin B 03')

                if (Acta.objects.filter(Q(pedido=pedido) & Q(item_cont='B 08')).count() == 0):
                    crear_novedad(pedido, 'B 07 sin B 08')

                try:
                    pedi = Acta.objects.get(
                        Q(pedido=pedido) & Q(item_cont='B 08'))
                    if pedi.cantidad != pedido.cantidad:
                        crear_novedad(pedido, 'Cantidades deben ser iguales. B 07 = ' +
                                      str(pedido.cantidad) + " B 08 = " + str(pedi.cantidad))
                except:
                    pass

            if pedido.item_cont == 'B 09':
                try:
                    pedi = Acta.objects.get(
        ...
        if pedido.item_cont == 'A 39':
            ...
            for b in busquedad_200410:
                if len(busquedad_200410) > 0:
                    if float(b.cantidad) < 7:
                        crear_novedad(
                            pedido, 'Cantidad de cable 200410 ó 200411 menor a 7')
        ...
        if pedido.item_cont == 'A 40':
            ...
            if Acta.objects.filter(pedido=pedido).filter(item_cont='200316').count() == 0:
                crear_novedad(pedido, str(pedido.item_cont) +
                              ". Sin cable 200316.")
...
def pedidos_doble_linea(pedido):

    lineas = Acta.objects.filter(pedido=pedido)
    lista = []
    for l in lineas:
        if l.item_cont not in lista:
            if Acta.objects.filter(pedido=pedido).filter(item_cont=l.item_cont).count() > 1:
                crear_novedad(pedido, ' ' + str(l.item_cont) +
                              ' con linea repetida.')
            lista.append(l.item_cont)
